...
const createAndSendToken = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    ...
    httpOnly: true, // cannot be accessed or modified in any way by the browser in order to prevent XSS
  };
  ...
  // hiding unwanted properties from response obj
...
exports.signup = catchAsync(async (req, res) => {
  ...
  const url = `${req.protocol}://${req.get('host')}/me`; // TEST
...
exports.isLoggedIn = async (req, res, next) => {
  if (req.cookies.jwt) {
    try {
      // 2. Verification(verifying) token
...
exports.protect = catchAsync(async (req, res, next) => {
  let token;
  // 1. Get the token and if it exists
  if (
  ...
  if (!token) {
    return next(
      new AppError('You are not logged in! Pleasae log in to get access', 401),
    );
  }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 3. Updating changePasswordAt property for the user (happens in a middleware)
